package com.ctrloption.bolareportservices.utils;
...
public class AppUtils {
    ...
    public static HeaderResponse setHeaderResponse(HeaderRequest request) {
        ...
        try {
            ...
            if (!AppUtils.validateRequestToken(request)) {
                msg = ResponseCodes.getAppMsg(ResponseCodes.INVOICE_NO_REQUIRED, request.getLang());
                response.setResponseCode(ResponseCodes.INVOICE_NO_REQUIRED);
                response.setResponseMessage(msg);
                msg = ResponseCodes.getAppMsg(ResponseCodes.INVOICE_NO_REQUIRED, "");
                AppLogger.warn(log, msg);
                return response;
            }
    ...
    public static boolean validateRequestToken(HeaderRequest request) {
        try {
            log.info("request id" + request.getRequestId() + "\taffcode" + request.getCompany() + "\tlang" + request.getLang());
            log.info("Client requestToken\t" + request.getRequestToken());
            String key = Settings.getInstance("").getProperty("SECRET_KEY");
            String originalString = request.getRequestId() + request.getCompany() + key + request.getLang();
            MessageDigest digest = MessageDigest.getInstance("SHA-512");
